...
function eatItem(socket, input, player){
    try {
        if (input === ""){
            ...
            for (const item of player.inventory){
                ...
                if ((itemName === input) && (item.quantity > 0)){
                    ...
                    socket.emit('eat', { target:itemName, itemId, player: player.characterName });
                    return true;
            ...
            if (potentialArray.length > 1){
                ...
                socket.emit('eat', { target:potentialArray[0], itemId, player: player.characterName });
                return true;
            } else {
