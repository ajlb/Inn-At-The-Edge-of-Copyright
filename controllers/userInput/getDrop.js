...
function giveItem(socket, io, target, item, itemId, user, location, players) {
        // How this works:
    ...
    for (let i = 2; i >= 0; i--) {
        const potentialTarget = target.toLowerCase().split(' ').slice(0, i + 1).join(' ');
        console.log("potential target:", potentialTarget);
        players.forEach(player => {
            if (player.toLowerCase() === potentialTarget || player.toLowerCase().startsWith(potentialTarget)) {
                console.log(`${socket.nickname}'s whisper found ${potentialTarget}`);
                target = player;
                playerExists = true;
            }
        })
